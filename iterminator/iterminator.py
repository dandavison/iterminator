...
class ColorSchemeSelector(object):
    ...
    def __init__(self, quiet=True):
        self.quiet = quiet
        self.repo_dir = os.path.join(os.path.dirname(__file__), "iTerm2-Color-Schemes")
    ...
    def filter_light_or_dark(self, is_light):
        self.schemes = deque(scheme for scheme in self.schemes if scheme.is_light() == is_light)
        self._post_change_schemes_hook()
    ...
    def apply(self):
        ...
        """
        subprocess.check_call([self.repo_dir + "/tools/preview.rb", self.scheme.path])
    ...
    def get_matches(self, query):
        ...
        return [name for name in self.scheme_names if match_operator(query, name)]
...
def parse_arguments():
    ...
    arg_parser.add_argument(
        "--dark", action="store_true", help="Restrict to dark background themes\n\n"
    )
    ...
    arg_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Select color scheme with tab-completion.\n"
            "Key bindings:\n"
            "tab         - complete\n"
    ...
    arg_parser.add_argument(
        "--light", action="store_true", help="Restrict to light background themes\n\n"
    )

    arg_parser.add_argument(
        "-l", "--list", action="store_true", help="List available color schemes\n\n"
    )
    ...
    arg_parser.add_argument(
        "-r", "--random", action="store_true", help="Select a random color scheme\n\n"
    )

    arg_parser.add_argument("-v", "--version", action="store_true", help="Show version\n\n")

    arg_parser.add_argument(
        "-s", "--scheme", help="Available choices are\n%s" % " | ".join(selector.scheme_names)
    )
...
def main():
    if os.getenv("TMUX"):
        error("Please detach from your tmux session before running this command.")
